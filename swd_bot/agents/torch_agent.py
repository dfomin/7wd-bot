...
class TorchAgent(Agent):
    ...
    def choose_action(self, state: GameState, possible_actions: Sequence[Action]) -> Action:
        ...
        action_predictions, _ = self.predict(state)
        action_probs = np.zeros(len(possible_actions))
        for i, action in enumerate(possible_actions):
            if isinstance(action, BuyCardAction):
                action_probs[i] = action_predictions[action.card_id]
            elif isinstance(action, DiscardCardAction):
                action_probs[i] = action_predictions[action.card_id + EntityManager.cards_count()]
            elif isinstance(action, BuildWonderAction):
                action_probs[i] = action_predictions[action.wonder_id + 2 * EntityManager.cards_count()]
            else:
                raise ValueError
